...
for blob in tqdm(client.list_blobs('theta-byte-342416-kubeflowpipelines-default', prefix='wonlost')):
    data = blob.download_as_string().decode("utf-8") 
    x = json.loads(data)
    all_data = pd.concat([all_data, pd.DataFrame.from_records([x])])

all_data.to_csv('./wonlost.csv')