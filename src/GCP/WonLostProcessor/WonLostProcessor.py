...
def computeWinLost(matchName):
    ...
    for blob in stclient.list_blobs('theta-byte-342416-kubeflowpipelines-default', prefix=matchName):
        ...
        query_job = bqclient.query(
            f'SELECT Traditional_Scoring_High,Traditional_Scoring_Low,Technical_Scoring,Autonomous_Scoring,Endgame,Fouls,Defense FROM `theta-byte-342416.unified.competitions_noqm` WHERE (Competition="{competition}" AND (Robot="{alliances["blue"]["team_keys"][0]}" OR Robot="{alliances["blue"]["team_keys"][1]}" OR Robot="{alliances["blue"]["team_keys"][2]}"))'
        )
        ...
        query_job = bqclient.query(
            f'SELECT Traditional_Scoring_High,Traditional_Scoring_Low,Technical_Scoring,Autonomous_Scoring,Endgame,Fouls,Defense FROM `theta-byte-342416.unified.competitions_noqm` WHERE (Competition="{competition}" AND (Robot="{alliances["red"]["team_keys"][0]}" OR Robot="{alliances["red"]["team_keys"][1]}" OR Robot="{alliances["red"]["team_keys"][2]}"))'
        )
        stats = list(query_job.result())
        if (len(stats) != 3):  raise ValueError(f'[{matchName}] Did not find 3 teams, but {len(stats)}: {", ".join(stats)}')
        ...
        res.update(kvb)
        res.update(kvr)
        job = bqclient.load_table_from_dataframe(pd.DataFrame(data=[res]), f'theta-byte-342416.wonlost_noqm.{os.path.basename(matchName)}')
