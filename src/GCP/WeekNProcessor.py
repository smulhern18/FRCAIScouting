...
event_name = '2019wowcmp'
year = event_name[0:4]

client = storage.Client.from_service_account_info(serviceAccount)
for blob in client.list_blobs('theta-byte-342416-kubeflowpipelines-default', prefix=f'pulled_matches/{year}/{event_name}_event_data'):
    filename = os.path.basename(blob.name)
    event_info = blob.download_as_string().decode("utf-8") 
    print(event_info)
    event_info = event_info.replace("\'", "\"")
    event_info = event_info.replace("None", "\"None\"")
