...
sourceTables = 'theta-byte-342416.unified'
dest_table_id = 'theta-byte-342416.unified.wonlost'
table_ids = []

query_job = client.query(
    f"""
    SELECT * 
    FROM {sourceTables}.INFORMATION_SCHEMA.TABLES
    """
)
...
schema = None
for idx, tableMeta in tqdm(enumerate(results)):
    if tableMeta[2] == 'competitions': continue
    table_ids.append(f'{sourceTables}.{tableMeta[2]}')
    # if (schema == None): schema = tableMeta[11]
    # if schema != tableMeta[11]: print(f'Wrong schema for {table_ids[-1]}')
    # print(json.dumps(tableMeta[11]))
...
# with open('wonlost.json', 'w') as f:
#     json.dump(table_ids, f)
#     exit()


# f = open ('wonlost.json', "r")
 
# table_ids = json.loads(f.read())

# batch_size = 1000
# for batch in range(0, len(table_ids), batch_size):
#     it = int((batch / batch_size))
#     print(f'{batch}-{(batch+batch_size - 1)}, items ')
#     if len(table_ids) < (batch+batch_size - 1):
#         print(f'Posting {len(table_ids[batch:])} items')
#         job = client.copy_table(table_ids[batch:], f'theta-byte-342416.unified.wonlost{it}')
#     else:
#         print(f'Posting {len(table_ids[batch:batch+batch_size - 1])} items')
#         job = client.copy_table(table_ids[batch:batch+batch_size - 1], f'theta-byte-342416.unified.wonlost{it}')
#     job.result()
